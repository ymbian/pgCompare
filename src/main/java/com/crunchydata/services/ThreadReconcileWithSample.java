 ...
 * @author Brian Pace
 */
public class ThreadReconcileWithSample extends threadReconcile{
    ...
    public ThreadReconcileWithSample(Properties Props, Integer threadNumber, DCTable dct, DCTableMap dctm, ColumnMetadata cm, Integer cid, ThreadSync ts, Boolean useDatabaseHash, String stagingTable, BlockingQueue<DataCompare[]> q) {
        super(Props,threadNumber,dct,dctm,cm,cid,ts,useDatabaseHash,stagingTable,q);
    ...
    public void run() {
        ...
        ResultSet samplePkResultSet = null;
        ResultSet rs = null;
        ...
        try {
            ...
            //todo
            long rowCount = getRowCount(repoConn, "sample_pk");
            long processedRecords = 0;

            while (processedRecords < rowCount) {
                //todo
                int offset=0;
                String getPkSQL = "SELECT pk from sample_pk  ORDER BY pk ASC LIMIT 2000 OFFSET ?";
                PreparedStatement queryPKPreparedStatement = repoConn.prepareStatement(getPkSQL);
                queryPKPreparedStatement.setInt(1,offset);

                 samplePkResultSet = queryPKPreparedStatement.executeQuery();

                // Connect to Source/Target
                Logging.write("info", threadName, String.format("(%s) Connecting to database", targetType));


                while (samplePkResultSet.next()) {
                    pkIds.add(samplePkResultSet.getLong(1));
                }
                ...
                if (!pkList.isEmpty() && Props.getProperty("database-sort").equals("true")) {
                    sql += " AND  " + pkList + " IN(";
                    // 使用参数化查询
                    String placeholders = pkIds.stream()
                            .map(id -> "?")
                            .collect(Collectors.joining(","));
                    sql +=  placeholders + ") ORDER BY " + pkList;
                }
            ...
            try {
                ...
                if (samplePkResultSet != null) {
                    samplePkResultSet.close();
                }
